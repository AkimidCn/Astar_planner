...
{   
    gl_xl = global_xyz_l(0); //地图下界 (-25,-25,0)
    gl_yl = global_xyz_l(1);
    gl_zl = global_xyz_l(2);

    gl_xu = global_xyz_u(0); //地图上界 (25,25,5)
...
Vector3d AstarPathFinder::gridIndex2coord(const Vector3i & index) 
{
    ...
    pt(0) = ((double)index(0) + 0.5) * resolution + gl_xl; // 0.5是为了取格子中心点
    ...
    for (int i = -1; i <= 1; i++)
    {
        for (int j = -1; j <= 1; j++)
        {
            for (int k = -1; k <= 1; k++)
            {
                ...
                if (isOccupied(n_x, n_y, n_z)) // 存在障碍物
                    continue;

                temp_ptr = GridNodeMap[n_x][n_y][n_z];

                if (temp_ptr->id == -1)  // 在closeset中
                    continue;
...
{   
    ...
    start_pt = gridIndex2coord(start_idx);  // 转换到网格中心位置
    ...
    */
    ...
    startPtr -> gScore = 0;
    startPtr -> fScore = getHeu(startPtr,endPtr);   // 获取启发h
    //STEP 1: finish the AstarPathFinder::getHeu , which is the heuristic function
    startPtr -> id = 1; 
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){  // 遍历周围八个邻居点
           ...
           neighborPtr = neighborPtrSets[i];
            if(neighborPtr -> id == 0){   // 邻居点不在openset也不在closeset,计算代价放入openset中
                /*
                *
                *
                ...
                */
                ...
                openSet.insert(make_pair(neighborPtr->fScore, neighborPtr)); // 放入openset中
                neighborPtr->id = 1;
                continue;
            }
...
{   
    ...
    */
    ...
    {
        gridPath.push_back(ptr); // 将当前节点加入路径
        ptr = ptr->cameFrom; // 移动到父节点
    }

    for (auto ptr: gridPath)  // 遍历容器（gridPath）中的数据
        path.push_back(ptr->coord);  // coord：世界坐标
