...
int main(int argc, char** argv)
{
    ...
    _map_sub  = nh.subscribe( "map",       1, rcvPointCloudCallBack );   // 订阅地图
    _pts_sub  = nh.subscribe( "waypoints", 1, rcvWaypointsCallback );  // 目标点
    ...
    _map_lower << - _x_size/2.0, - _y_size/2.0,     0.0;  // << Eigen库的流插入操作符，用于初始化向量/矩阵  (-25,-25,0)
    _map_upper << + _x_size/2.0, + _y_size/2.0, _z_size; // (25,25,5)
    ...
    //_astar_path_finder  = new AstarPathFinder();  //可能导致内存泄漏
